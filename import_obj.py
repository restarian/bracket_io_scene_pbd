...
def create_mesh(new_objects,
    ...
    # ob.pbd_prop.draw_order = int(draw_order # )
    for key in custom_property[dataname]:
        ob.pbd_prop[key] = custom_property[dataname][key]
    ...
    with ProgressReport(context.window_manager) as progress:
        ...
        with open(filepath, 'rb') as f:
            for line in f:
                ...
                if vdata_len:
                ...
                elif line_start == b'display':
                    if use_split_objects:
                        key = context_object_key.decode("UTF-8")
                        if not key in custom_property:
                            custom_property[key] = {}
                        custom_property[key]["display"] = bool(int(line_value(line_split).lower()))

                elif line_start.decode("UTF-8").startswith("header_"):
                    li = line_start.decode("UTF-8")[7:]
                    context.scene.pbd_prop[li] = line_value(line_split).decode("UTF-8").strip()

                elif line_start == b'clipregion':
                    if use_split_objects:
                        key = context_object_key.decode("UTF-8")
                        if not key in custom_property:
                            custom_property[key] = {}
                        li = line_value(line_split).decode("UTF-8").lower().strip()
                        custom_property[key]["clip_region"] = bool(li != 'off')
                        custom_property[key]["clip_bounding"] = bool(li == 'bounding')

                elif line_start == b'mouseregion':
                    if use_split_objects:
                        key = context_object_key.decode("UTF-8")
                        if not key in custom_property:
                            custom_property[key] = {}
                        li = line_value(line_split).decode("UTF-8").lower().strip()
                        custom_property[key]["mouse_region"] = bool(li != 'off')
                        custom_property[key]["mouse_bounding"] = bool(li == 'bounding')
                    ...
                    if use_split_objects:
                        ...
                        if not key in custom_property:
                            custom_property[key] = {}
                        custom_property[key]["cull_face"] = str(line_value(line_split).decode("UTF-8").lower().strip())
        ...
        for data in split_mesh(verts_loc, faces, unique_materials, filepath, SPLIT_OB_OR_GROUP):
            ...
            #props = custom_property[dataname]
